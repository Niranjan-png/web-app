...
// Send SMS alert after delay
async function sendDelayedSMS(message) {
  // Clear any existing timeout
  if (smsTimeout) {
    clearTimeout(smsTimeout);
  }

  // Set new timeout for SMS sending
  smsTimeout = setTimeout(async () => {
    try {
      const twilioClient = new Twilio(TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN);
      const phoneNumber = caregiverData?.phone;
      
      if (!phoneNumber) {
        console.error('No caregiver phone number set');
        return;
      }
